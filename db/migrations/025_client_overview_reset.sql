-- Стратегия сброса и дата следующего — в обзор клиента.
--
-- Счётчик трафика обнуляется по расписанию тарифа, но в панели об этом не
-- было сказано нигде: человек видел «использовано 0» и решал, что счётчик
-- сломался, — хотя сброс отработал по расписанию час назад. Показываем и
-- стратегию, и момент следующего сброса рядом с самим счётчиком.
--
-- Владельца задаём явно: пересоздание вида под другой ролью уже отбирало
-- у приложения право на SELECT.
...
CREATE OR REPLACE VIEW client_overview AS
 SELECT c.id,
    ...
    COALESCE(( SELECT sum(pm.amount_minor)::bigint AS sum
           FROM payments pm
          WHERE pm.client_id = c.id AND pm.status = 'success'::payment_status), 0::bigint) AS ltv_minor,
    ( SELECT count(*) AS count
           FROM payments pm
