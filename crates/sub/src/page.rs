...
fn platform_title(p: &str) -> &'static str {
    match p {
        "ios" => "iPhone · iPad",
        "android" => "Android",
        "windows" => "Windows",
...
pub fn render(
    ...
    let used = sn_core::money::format_bytes(d.used);
    let limit = d
        .limit
        .map(sn_core::money::format_bytes)
    ...
    let expires = d
        ...
        .unwrap_or_else(|| "Без срока".into());
    let days = d
        .expires_at
        .map(|e| humanize_days((e - chrono::Utc::now()).num_days()))
        .unwrap_or_else(|| "Без ограничения срока".into());
    ...
    let devices_summary = if d.device_limit > 0 {
        ...
        r#"<span role="img" aria-label="Безлимит" title="Без ограничения устройств">∞</span>"#.into()
    };
    let (state, heading, explanation) = match d.status.as_str() {
        "active" if ready => (
            "Доступ активен",
            "Подключим ваше устройство",
            "Установите приложение и добавьте подписку — всё готово для подключения.",
        ),
        "active" => (
            "Доступ активен",
            "Локации готовятся",
            "В подписке пока нет доступных локаций.",
        ),
        "limited" => (
            "Лимит трафика",
            "Трафик закончился",
            "Подключение появится после сброса лимита или продления.",
        ),
        "disabled" => (
            "Доступ отключён",
            "Подписка отключена",
            "Свяжитесь с поддержкой, чтобы уточнить причину.",
        ),
        _ => (
            "Срок истёк",
            "Подписка завершилась",
            "После продления здесь снова появится подключение.",
        ),
    };
    ...
    let support_action = support.map(|u| format!(r#"<a class="button primary" href="{}" target="_blank" rel="noopener">{}Написать в поддержку</a>"#,esc(u),icon_send())).unwrap_or_else(|| "<p class=\"muted\">Обратитесь к администратору вашего VPN-сервиса.</p>".into());
    let support_footer = support
        ...
        .unwrap_or_else(|| "<span>Настройка безопасного подключения</span>".into());
    ...
    let platform_options = ["ios", "android", "windows", "macos", "linux"]
        ...
        .collect::<String>();
    let choices = apps.iter().enumerate().map(|(i,a)|format!(r#"<button type="button" class="app-choice" data-choice="{i}" data-platform="{platform}" aria-pressed="false"><span class="app-icon">{icon}</span><span class="app-label"><b>{name}</b><span>{hint}</span></span><span class="choice-check" aria-hidden="true">{check}</span></button>"#,platform=esc(&a.platform),icon=render_icon(a),name=esc(&a.name),hint=if a.recommended {"Рекомендуем"} else {"Выбрать приложение"},check=icon_check())).collect::<String>();
    let application_views = apps.iter().enumerate().map(|(i,a)|format!(r#"<div class="application" data-app="{i}" data-platform="{platform}" data-recommended="{recommended}" {hidden}>{content}</div>"#,platform=esc(&a.platform),recommended=a.recommended,hidden=if i==initial {""}else{"hidden"},content=render_app(a,sub_url))).collect::<String>();
    let app_picker = apps.get(initial).map(|a| format!(r#"<button class="app-picker" id="appPicker" type="button" data-sheet="appsSheet" aria-haspopup="dialog" aria-label="Выбрать приложение"><span class="app-icon">{}</span><span class="app-label"><b>{}</b><span>Выбрать другое приложение</span></span>{}</button>"#,render_icon(a),esc(&a.name),icon_chevron())).unwrap_or_default();
    let connect = if ready && !apps.is_empty() {
        format!(
            r#"<div class="device-row"><label for="deviceSelect">Ваше устройство</label><select id="deviceSelect">{platform_options}</select></div>{app_picker}<div id="applicationViews">{application_views}</div>"#
        )
    } else if ready {
        format!(
            r#"<div class="manual-state">{}<h2>Добавьте подписку вручную</h2><p>Скопируйте ссылку и импортируйте её в своё VPN-приложение.</p><button class="button primary" type="button" data-copy>Скопировать ссылку</button><button class="text-button" type="button" data-sheet="helpSheet">Как это сделать</button></div>"#,
            icon_device()
        )
    } else {
        format!(
            r#"<div class="access-state">{}<h2>{}</h2><p>{}</p>{support_action}<button class="text-button" type="button" data-sheet="accountSheet">Подробнее о подписке</button></div>"#,
            icon_lock(),
            if active {
                "Нужна помощь с локациями"
            } else {
                "Восстановим доступ"
            },
            if active {
                "Поддержка проверит назначенные вам серверы."
            } else {
                "Управление подпиской доступно через ваш сервис."
            }
        )
    };
    ...
    let server_sheet = if active {
        sheet("serversSheet", "Ваши локации", &render_servers(d))
    } else {
        String::new()
    };
    let extra_tools = if active {
        format!(
            r#"<button class="quick-action" type="button" data-sheet="serversSheet">{}<span>Локации <b>{}</b></span></button><button class="quick-action" type="button" data-sheet="linkSheet">{}<span>Ссылка и QR</span></button>"#,
            icon_servers(),
            d.hosts.len(),
            icon_qr()
        )
        ...
        format!(
            r#"<button class="quick-action" type="button" data-sheet="helpSheet">{}<span>Помощь</span></button>"#,
            icon_info()
        )
    };
    let notice_button = if notice.is_empty() {
        ...
        format!(
            r#"<button class="service-note" type="button" data-sheet="noticeSheet">{}<span>Сообщение сервиса</span>{}</button>"#,
            icon_alert(),
            icon_arrow()
        )
    };
    ...
    let account = sheet(
        ...
        "Ваша подписка",
        &format!(
            r#"<p class="account-user">{user}</p><span class="status {tone}">{state}</span><dl class="account-facts"><dt>Тариф</dt><dd>{tariff}</dd><dt>Действует до</dt><dd>{expires}</dd><dt>Остаток срока</dt><dd>{days}</dd><dt>Трафик использован</dt><dd>{used}</dd><dt>Лимит трафика</dt><dd>{limit}</dd><dt>Лимит устройств</dt><dd>{devices}</dd></dl>{traffic}<p class="prose">{footer}</p>"#,
            user = esc(&d.username),
            tone = if active { "ok" } else { "attention" },
            tariff = esc(d.tariff.as_deref().unwrap_or("Не указан")),
            days = esc(&days),
            footer = esc(settings.footer_note.as_deref().unwrap_or("")),
            traffic = if d.limit.is_some() {
                format!(
                    r#"<div class="traffic-label">Использовано {percent:.0}% трафика</div><div class="traffic-bar"><i style="width:{percent:.1}%"></i></div>"#
                )
            } else {
                String::new()
            }
    ...
    let manual_install="Установите совместимое VPN-приложение из официального источника. Если не знаете, какое выбрать, обратитесь в поддержку.";
    let manual_import="Нажмите «Скопировать ссылку» на главном экране, откройте импорт из буфера обмена в VPN-приложении и подтвердите добавление профиля.";
    let steps=if ready {format!(r#"<ol class="guide-steps"><li><b>Установите приложение</b><p data-help-install>{manual_install}</p></li><li><b>Добавьте подписку</b><p data-help-import>{manual_import}</p></li><li><b>Включите VPN в приложении</b><p>Выберите локацию, нажмите кнопку подключения и разрешите создание VPN-подключения, если система попросит.</p></li></ol>"#)}else{format!(r#"<p class="sheet-lead">{}</p>"#,esc(explanation))};
    let help = sheet(
        "helpSheet",
        "Помощь с подключением",
        &format!(
            r#"{steps}<div class="faq"><details><summary>Открыли страницу внутри Telegram?</summary><p>Если кнопка не открывает приложение, выберите «Открыть в браузере» в меню Telegram и повторите.</p></details><details><summary>Локации не подключаются</summary><p>Обновите приложение и подписку, проверьте срок и трафик. Попробуйте другую локацию или сеть. Передайте поддержке название приложения, локацию и текст ошибки.</p></details><details><summary>Можно поделиться ссылкой?</summary><p>Ссылка даёт доступ к вашей подписке. Чужие устройства могут занять лимит и расходовать трафик. Для другого человека нужна отдельная подписка.</p></details></div>{support_action}"#
        ),
    );
    let no_script = if active {
        format!(
            r#"<noscript><p>Для выбора приложений включите JavaScript. <a href="{}">Открыть конфигурацию подписки</a>. Ссылку можно скопировать из адресной строки.</p></noscript>"#,
            esc(sub_url)
        )
    } else {
        String::new()
...
<script>{js}</script></body></html>"##,
        brand = esc(brand),
        title = esc(settings.title.as_deref().unwrap_or("Подключение")),
        ...
        theme_sheet = sheet("themeSheet","Оформление",&format!(r#"<p class="sheet-lead">Выберите тему или используйте оформление устройства.</p><fieldset class="theme-options"><legend class="visually-hidden">Тема страницы</legend>{}</fieldset>"#,[ ("auto","Автоматически","Как на вашем устройстве"),("light","Светлая","Светлые карточки и мягкие тени"),("dark","Тёмная","Мягкий контраст на тёмном фоне") ].iter().map(|(id,name,hint)|format!(r#"<label class="theme-option"><input type="radio" name="theme" value="{id}"><span><b>{name}</b><small>{hint}</small></span>{}</label>"#,icon_check())).collect::<String>())),
        help_icon = icon_info(),
        ...
        apps_sheet = if ready && !apps.is_empty() {
            sheet(
                ...
                "Выберите приложение",
                &format!(
                    r#"<p class="sheet-lead">Для <span id="appsPlatform">вашего устройства</span>. Выберите одно приложение, которое будете использовать.</p><div class="app-choices">{choices}</div>"#
                ),
            )
        } else {
            String::new()
...
fn sheet(id: &str, title: &str, content: &str) -> String {
    format!(
        r#"<dialog class="sheet" id="{id}" aria-labelledby="{id}Title"><header class="sheet-header"><h2 id="{id}Title">{title}</h2><button class="close-button" type="button" data-close aria-label="Закрыть окно" autofocus><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m6 6 12 12M18 6 6 18"/></svg></button></header><div class="sheet-body">{content}</div></dialog>"#
    )
}

fn render_servers(d: &SubData) -> String {
    if d.status != "active" {
        return String::new();
    }
    let rows = d.hosts.iter().map(|h|format!(r#"<details class="server"><summary><span><b>{}</b><small>{} · {}</small></span>{}</summary><dl><dt>Адрес</dt><dd>{}:{}</dd><dt>Протокол</dt><dd>{}</dd><dt>Транспорт</dt><dd>{}</dd><dt>Защита</dt><dd>{}</dd></dl></details>"#,esc(&h.remark),esc(&h.protocol.to_uppercase()),esc(&h.security.to_uppercase()),icon_chevron(),esc(&h.address),h.port,esc(&h.protocol),esc(&h.network),esc(&h.security))).collect::<String>();
    format!(
        r#"<p class="sheet-lead">Выбирайте локацию внутри VPN-приложения. Задержку можно измерить там же — она зависит от вашей сети.</p><div class="server-list">{}</div>"#,
        if rows.is_empty() {
            "<p>Доступных локаций пока нет. Обратитесь в поддержку.</p>".into()
        } else {
            rows
        }
    )
...
fn render_app(a: &App, sub_url: &str) -> String {
    ...
    let hint = if deeplink.is_some() {
        "Затем включите VPN в приложении."
    } else {
        "Импортируйте ссылку в приложение и включите VPN."
    };
    let install = a.store_url.as_deref().filter(|v|!v.is_empty()).map(|url|format!(r#"<a class="button install" href="{}" target="_blank" rel="noopener"><span class="step-number">1</span><span>Установить {}</span>{}</a>"#,esc(url),esc(&a.name),icon_external())).unwrap_or_else(||format!(r#"<p class="manual-install">Установите {} из официального источника.</p>"#,esc(&a.name)));
    let add = deeplink.map(|url|format!(r#"<a class="button primary import" href="{}" data-import><span class="step-number">2</span><span>Добавить подписку</span>{}</a>"#,esc(&url),icon_arrow())).unwrap_or_else(||format!(r#"<button class="button primary" type="button" data-copy><span class="step-number">2</span><span>Скопировать ссылку</span>{}</button>"#,icon_link()));
    let guide = a.guide.as_deref().filter(|v|!v.is_empty()).map(|text|format!(r#"<details class="app-guide"><summary>Инструкция для {}</summary><p class="prose">{}</p></details>"#,esc(&a.name),esc(text))).unwrap_or_default();
...
mod tests {
    ...
    fn плейсхолдер_url_подставляется() {
        ...
        let html = render_app(&app, "https://sub.example/s/abc");
    ...
    fn срок_склоняется_по_русски() {
        assert_eq!(humanize_days(1), "остался 1 день");
        assert_eq!(humanize_days(2), "осталось 2 дня");
        assert_eq!(humanize_days(5), "осталось 5 дней");
        assert_eq!(humanize_days(11), "осталось 11 дней");
        assert_eq!(humanize_days(21), "осталось 21 день");
        assert_eq!(humanize_days(0), "истекает сегодня");
        assert_eq!(humanize_days(-1), "истекла");
        assert_eq!(humanize_days(60), "осталось ~2 месяца");
        assert_eq!(humanize_days(180), "осталось ~6 месяцев");
        assert_eq!(humanize_days(31), "осталось ~1 месяц");
    }
}
