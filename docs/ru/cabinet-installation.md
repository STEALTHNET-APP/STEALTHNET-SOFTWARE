...
При обновлении готовой установки до v0.2.5 и новее стандартный кабинет на сервере этой же панели обновляется вместе с ней:
...
Кабинет **на другом сервере** обновляется на его сервере после панели:

```bash
/usr/local/sbin/update-cabinet
```

Эта отдельная команда также доступна для нестандартного размещения и установки панели из исходников. Команда относится к кабинету и Mini App. Страница подписки и список приложений обслуживаются сервисом подписки.
