...
3. Заполните вкладку **Параметры**. Массивы `serverNames` и `shortIds` вводятся как JSON, например `["vpn.example.com"]` и `["0123456789abcdef"]`.
4. При необходимости нажмите **Заполнить недостающие ключи**. Генерируются отсутствующие ключи Reality и серверные ключи Shadowsocks 2022. Введённые ключи не заменяются. Домены, target, сертификаты и данные следующего сервера указывает владелец.
5. Откройте **JSON**, проверьте полный документ. Обратное переключение в форму сохраняет остальные поля.
6. Нажмите **Проверить**. Список незаполненных параметров содержит кнопки перехода к полям. С исправленными ошибками создайте профиль.
