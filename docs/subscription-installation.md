...
| Установка | Уже входит в `install.sh` и Compose проекта | Команда в Настройки → Сервис подписки |
...
Если панель установлена через `install.sh`, `sn-sub` уже установлен и читает ту же базу. Сервис использует общий `.env` из каталога проекта, заданный в `EnvironmentFile` его systemd-юнита. Не заменяйте этот юнит инструкциями для отдельной машины.
...
Для новой установки всей панели запустите `sudo bash install.sh` из каталога исходников проекта. Скрипт спросит два домена, установит оба сервиса и создаст конфигурацию Caddy. Не запускайте `install-sub.sh` поверх существующей установки панели.
...
Если бинарник нужной архитектуры ещё не опубликован, установка остановится. Нужно собрать `sn-sub` для Linux этой архитектуры и разместить `sn-sub-linux-amd64` или `sn-sub-linux-arm64` в каталоге сайта панели. Установка панели публикует сборку архитектуры своего сервера. Также можно задать `SUB_BINARY_URL` с адресом своей сборки. Скрипт не принимает HTML вместо бинарника.
...
`/health` означает только, что процесс отвечает. `/ready` проверяет доступ к базе или API. Ни один из этих адресов сам по себе не проверяет доступ клиента к VPN-ноде. Запрос `/s/НЕСУЩЕСТВУЮЩИЙ_ID` должен вернуть 404; это не проверка действующей подписки.
...
**Отдельный сервер:** сначала обновите панель, затем на сервере подписки выполните:

```bash
env PANEL_URL='https://panel.example.com' bash -o pipefail -c 'curl -fsSL "$PANEL_URL/update-sub.sh" | bash'
```
