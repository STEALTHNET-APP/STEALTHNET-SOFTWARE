...
const ProfileWorkshop = (() => {
  ...
  function parameters(cfg){
    ...
    if(cfg._selfsteal){
      const group=text('Ваш сайт на ноде','Your node website'),groupId='selfsteal';
      add('/_selfsteal/domain',text('Домен вашего сайта','Your website domain'),text('Например, nl.example.com. A/AAAA должны указывать на эту ноду. Домен панели, IP и ссылку на подписку сюда вводить не нужно.','For example, nl.example.com. Its A/AAAA records must point to this node. Do not enter the panel domain, an IP address or a subscription URL.'),{group,groupId,kind:'selfsteal_domain',required:true,example:'nl.example.com'});
      add('/_selfsteal/title',text('Название сайта','Website title'),text('До 80 символов. Оставьте пустым, чтобы использовать название выбранного мини-сайта.','Up to 80 characters. Leave blank to use the selected mini-site’s title.'),{group,groupId,maxlength:80});
      add('/_selfsteal/description',text('Текст на главной странице','Homepage text'),text('До 600 символов. Пустое поле использует текст выбранного мини-сайта.','Up to 600 characters. Leave blank to use the selected mini-site’s text.'),{group,groupId,kind:'multiline',maxlength:600});
    }
    (Array.isArray(cfg.inbounds)?cfg.inbounds:[]).forEach((ib,i)=>{
      ...
      const own=cfg._selfsteal?.inbound_tag===ib.tag;
    ...
    for(const path of leaves(cfg)){
      ...
      const ownIndex=cfg._selfsteal?(cfg.inbounds||[]).findIndex(i=>i.tag===cfg._selfsteal.inbound_tag):-1;
      if(ownIndex>=0&&path.startsWith('/inbounds/'+ownIndex+'/streamSettings/realitySettings/')&&/\/(target|dest|serverNames)(\/|$)/.test(path))continue;
  ...
  function applyFields(cfg,params,values){
    ...
    for(const {index,value} of values){
      ...
      try{
        let parsed=value;
        if(p.kind==='selfsteal_domain'){
          parsed=value.trim()?websiteAddress(value).host:'';
          if(value.trim()&&websiteAddress(value).target!==parsed+':443')throw Error(text('Укажите домен без порта. Selfsteal использует HTTPS на 443.','Enter a domain without a port. Selfsteal uses HTTPS on 443.'));
    ...
    if(next._selfsteal)SelfstealUI.bind(next);
    return next;
  }
  ...
  function builtins(){const self=JSON.parse(PROFILE_PRESETS.find(p=>p.id==='reality-tcp').config),ib=self.inbounds.find(i=>i.protocol==='vless');self._selfsteal=SelfstealUI.defaults(ib.tag);SelfstealUI.bind(self);return PROFILE_PRESETS.map(p=>{
  ...
  }).concat([{id:'selfsteal',name:text('VLESS REALITY · свой сайт на ноде','VLESS REALITY · own node website'),group:'direct',builtin:true,version:'2026.09',description:text('Девять мини-сайтов на выбор, свой домен и автоматический HTTPS.','Nine mini-sites to choose from, your own domain and automatic HTTPS.'),config:self},{
  ...
  function requirements(cfg){const lines=new Set();for(const i of cfg.inbounds||[]){const s=i.streamSettings||{};
    if(cfg._selfsteal?.inbound_tag===i.tag){lines.add(text('Selfsteal: домен должен вести на эту ноду. Откройте TCP 80 для выпуска и обновления сертификата и TCP 443 для сайта и VPN. Агент подготовит сайт до переключения конфигурации. Сначала обновите агент ноды.','Selfsteal: point the domain to this node. Allow TCP 80 for certificate issuance and renewal, and TCP 443 for the website and VPN. The agent prepares the website before switching the configuration. Update the node agent first.'));continue;}
      ...
      onMount(l,close){
        ...
        function redraw(){
          ...
          const renderField=(p,i)=>{
            ...
            const limits=p.maxlength?`maxlength="${p.maxlength}"`:'';
            const input=['list','multiline'].includes(p.kind)?`<textarea class="inp pw-list" rows="2" ${attrs} ${limits}>${esc(value)}</textarea>`:`<input class="inp" type="${p.kind==='password'?'password':p.kind==='number'?'number':'text'}" ${p.kind==='number'?'min="1" max="65535" step="1"':''} ${attrs} ${limits} value="${esc(value)}">`;
          ...
          SelfstealUI.mount(q('#pwSelfsteal'),config,{read,isBusy:()=>busy,change(next){if(busy)return;config=next;dirty=true;changed=true;redraw();msg(result,text('Есть непроверенные изменения','Unvalidated changes'));}});
